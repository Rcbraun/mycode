...
import bootstrapper.py
# python3 -m pip install --user netmiko
from netmiko import ConnectHandler  ##custon class objest that is a ssh connection to a remote arista device
...
def main():
    ...
    if ynchk.lower() in ["y", "yes"]:
        ...
        if bootstrapper.bootstrapper(f"{entry[switchip]}",x, "admin",'alta3', conf_loc):
